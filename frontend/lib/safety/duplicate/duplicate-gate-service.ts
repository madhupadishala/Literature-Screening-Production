...
export interface DuplicateCandidateRecord {
  ...
  rank: number;
  humanCandidateDecision: "PENDING" | "NOT_MATCH" | "FOLLOW_UP_MATCH" | "DUPLICATE_MATCH";
}
...
function extractSourceIdentifiers(payload: unknown): string[] {
  ...
  function walk(value: unknown): void {
    ...
    for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
      ...
      if (allowed.has(normalizedKey) && (typeof item === "string" || typeof item === "number")) {
...
async function ensureGateEligible(
  client: PoolClient,
  tenantId: string,
  ...
  if (String(row.source_review_status) !== "VERIFIED") {
    throw new Error("Source Review must be VERIFIED before duplicate/follow-up review.");
  }
  return row;
}
...
async function loadFingerprints(
  ...
  const [patients, reporters, products, events] = await Promise.all([
    client.query<Record<string, unknown>>(
      `SELECT intake_record_id, patient_reference, sex, date_of_birth::text, age_value, age_unit
         FROM safety_patients
        WHERE tenant_id = $1 AND intake_record_id = ANY($2::uuid[])
...
async function loadCandidatePool(
  ...
  const candidates = await client.query<{ id: string }>(
    `SELECT intake.id
      ...
      WHERE intake.tenant_id = $1
        AND intake.id <> $2
        AND intake.source_review_status = 'VERIFIED'
  ...
  return loadFingerprints(client, tenantId, candidates.rows.map((item) => item.id));
}

function systemRecommendation(
  candidates: DuplicateMatchResult[],
): "NO_LIKELY_MATCH" | "POTENTIAL_MATCH" {
  return (candidates[0]?.score ?? 0) >= 50 ? "POTENTIAL_MATCH" : "NO_LIKELY_MATCH";
}

function mapRun(row: Record<string, unknown>): DuplicateReviewRunRecord {
  return {
    id: String(row.id),
...
function mapCandidate(row: Record<string, unknown>): DuplicateCandidateRecord {
  return {
    ...
    candidateSnapshot: object(row.candidate_snapshot) as unknown as DuplicateFingerprint,
    rank: Number(row.rank),
    humanCandidateDecision: String(row.human_candidate_decision) as DuplicateCandidateRecord["humanCandidateDecision"],
  };
}
...
function mapAssessment(row: Record<string, unknown>): DuplicateAssessmentRecord {
  return {
    ...
    systemRecommendation: String(row.system_recommendation) as DuplicateAssessmentRecord["systemRecommendation"],
    topCandidateScore: row.top_candidate_score === null || row.top_candidate_score === undefined ? null : Number(row.top_candidate_score),
    humanDecision: String(row.human_decision) as DuplicateHumanDecision,
    selectedCandidateId: row.selected_candidate_id ? String(row.selected_candidate_id) : null,
    selectedCandidateIntakeRecordId: row.selected_candidate_intake_record_id ? String(row.selected_candidate_intake_record_id) : null,
    selectedCandidateCaseId: row.selected_candidate_case_id ? String(row.selected_candidate_case_id) : null,
    selectedExternalReference: row.selected_external_reference ? String(row.selected_external_reference) : null,
    rationale: String(row.rationale),
    assessedAt: new Date(String(row.assessed_at)).toISOString(),
  };
}

export async function getDuplicateGateWorkspace(input: {
  ...
  try {
    await ensureGateEligible(client, input.principal.tenantId, intakeRecordId);
    const source = (await loadFingerprints(client, input.principal.tenantId, [intakeRecordId]))[0];
    ...
    const latestRun = runResult.rows[0] ? mapRun(runResult.rows[0]) : null;

    const candidateResult = latestRun
      ? await client.query<Record<string, unknown>>(
          `SELECT * FROM safety_duplicate_candidates
            WHERE tenant_id = $1 AND intake_record_id = $2 AND review_run_id = $3
            ORDER BY rank`,
    ...
    return {
      ...
      latestAssessment: assessmentResult.rows[0] ? mapAssessment(assessmentResult.rows[0]) : null,
    };
  } finally {
    client.release();
  }
...
export async function runDuplicateGateSearch(input: {
  principal: RequestPrincipal;
  intakeRecordId: string;
  reason: string;
}): Promise<DuplicateWorkspace> {
  ...
  if (!intakeRecordId) throw new Error("intakeRecordId is required.");
  if (reason.length < 10) throw new Error("Duplicate-search reason must contain at least 10 characters.");

  const client = await getPostgresPool().connect();
  let runId = "";
  try {
    await client.query("BEGIN");
    const intake = await ensureGateEligible(client, input.principal.tenantId, intakeRecordId, true);
    if (String(intake.duplicate_review_status) === "COMPLETE") {
      throw new Error("Completed duplicate review requires a controlled reopen before rerunning.");
    }
    ...
    const inserted = await client.query<{ id: string }>(
      `INSERT INTO safety_duplicate_review_runs (
         tenant_id,intake_record_id,run_number,algorithm_key,algorithm_version,
         candidate_threshold,source_snapshot,source_snapshot_sha256,status,created_by
    ...
    const ranked = rankDuplicateCandidates(
      source,
      await loadCandidatePool(client, input.principal.tenantId, intakeRecordId),
      { threshold: DUPLICATE_CANDIDATE_THRESHOLD, limit: 10 },
    );

    for (const [index, result] of ranked.entries()) {
      await client.query(
        `INSERT INTO safety_duplicate_candidates (
           tenant_id,intake_record_id,review_run_id,candidate_intake_record_id,
           candidate_case_id,candidate_reference,score,confidence_band,
           matched_factors,candidate_snapshot,rank
    ...
    await client.query(
      `UPDATE safety_duplicate_review_runs
          SET status='COMPLETED', candidate_count=$3, completed_at=now()
        WHERE tenant_id=$1 AND id=$2`,
      [input.principal.tenantId, runId, ranked.length],
    );
    ...
    await client.query(
      ...
      [
        input.principal.tenantId,
        intakeRecordId,
        systemRecommendation(ranked) === "POTENTIAL_MATCH" ? "POTENTIAL_DUPLICATE" : "UNASSESSED",
        input.principal.userId,
      ],
    );
    ...
    await client.query(
      `INSERT INTO safety_review_tasks (
         tenant_id,task_key,entity_type,entity_id,task_type,status,created_by
       ) VALUES ($1,$2,'INTAKE_RECORD',$3,'DUPLICATE_REVIEW','OPEN',$4)
       ON CONFLICT (tenant_id,task_key) DO NOTHING`,
      [input.principal.tenantId, `duplicate-review:${intakeRecordId}`, intakeRecordId, input.principal.userId],
    );

    await client.query(
      `INSERT INTO audit_events (
         tenant_id,actor_id,event_type,event_category,outcome,details
       ) VALUES ($1,$2,'DUPLICATE_GATE_SEARCH_COMPLETED','NEXUS_DUPLICATE_REVIEW','success',$3::jsonb)`,
      [
        input.principal.tenantId,
        input.principal.userId,
        JSON.stringify({
          ...
          topCandidateScore: ranked[0]?.score ?? null,
          systemRecommendation: systemRecommendation(ranked),
          reason,
        }),
      ],
    );
  ...
  return getDuplicateGateWorkspace({ principal: input.principal, intakeRecordId });
}

export async function finalizeDuplicateGateReview(input: {
  principal: RequestPrincipal;
  intakeRecordId: string;
  humanDecision: DuplicateHumanDecision;
  selectedCandidateId?: string | null;
  ...
  if (!intakeRecordId) throw new Error("intakeRecordId is required.");
  if (!["NEW_CASE", "FOLLOW_UP", "DUPLICATE", "NOT_MATCH"].includes(input.humanDecision)) {
    throw new Error("A valid duplicate/follow-up human decision is required.");
  }
  if (rationale.length < 10) throw new Error("Duplicate-review rationale must contain at least 10 characters.");
  ...
  try {
    ...
    const runResult = await client.query<Record<string, unknown>>(
      `SELECT * FROM safety_duplicate_review_runs
        WHERE tenant_id=$1 AND intake_record_id=$2 AND status='COMPLETED'
        ORDER BY run_number DESC LIMIT 1 FOR UPDATE`,
      [input.principal.tenantId, intakeRecordId],
    );
    const run = runResult.rows[0];
    if (!run) throw new Error("Run duplicate search before finalizing duplicate review.");

    const candidates = await client.query<Record<string, unknown>>(
      `SELECT * FROM safety_duplicate_candidates
        WHERE tenant_id=$1 AND intake_record_id=$2 AND review_run_id=$3
        ORDER BY rank FOR UPDATE`,
    ...
    if (input.humanDecision === "FOLLOW_UP" || input.humanDecision === "DUPLICATE") {
      ...
      if (!selectedCandidateId) throw new Error("A selected candidate is required for FOLLOW_UP or DUPLICATE.");
      selected = candidates.rows.find((item) => String(item.id) === selectedCandidateId) ?? null;
      if (!selected) throw new Error("Selected candidate does not belong to the latest duplicate-search run.");
      if (input.humanDecision === "FOLLOW_UP" && !selected.candidate_case_id) {
        throw new Error("A FOLLOW_UP must link to an existing Nexus case. Select a candidate with an existing case.");
      }
    }

    const selectedId = selected ? String(selected.id) : null;
    for (const candidate of candidates.rows) {
      const isSelected = selectedId !== null && String(candidate.id) === selectedId;
      ...
      await client.query(
        `UPDATE safety_duplicate_candidates
            SET human_candidate_decision=$4, reviewed_by=$5, review_reason=$6, reviewed_at=now()
        ...
        [
          ...
          candidateDecision,
          input.principal.userId,
          isSelected ? rationale : "Not selected by the final duplicate/follow-up assessment.",
        ],
      );
    }

    const topScore = candidates.rows.length ? Number(candidates.rows[0].score) : null;
    const recommendation: "NO_LIKELY_MATCH" | "POTENTIAL_MATCH" =
      topScore !== null && topScore >= 50 ? "POTENTIAL_MATCH" : "NO_LIKELY_MATCH";
    ...
    const assessment = await client.query<Record<string, unknown>>(
      `INSERT INTO safety_duplicate_assessments (
         ...
         selected_candidate_case_id,selected_external_reference,rationale,assessed_by
    ...
    const isDuplicate = input.humanDecision === "DUPLICATE";
    const nextStatus = isDuplicate ? "READY_FOR_DISPOSITION" : "IN_TRIAGE";
    ...
    await client.query(
      ...
      [
        input.principal.tenantId,
        intakeRecordId,
        isDuplicate ? "CONFIRMED_DUPLICATE" : "UNIQUE",
        input.humanDecision,
        selected?.candidate_case_id ? String(selected.candidate_case_id) : null,
        selected ? String(selected.candidate_intake_record_id) : null,
        text(selectedSnapshot.externalReference),
    ...
    await client.query(
      `UPDATE safety_review_tasks
          SET status='COMPLETED', completed_at=now(), outcome=$3::jsonb, updated_at=now()
        WHERE tenant_id=$1 AND entity_type='INTAKE_RECORD' AND entity_id=$2
          AND task_type='DUPLICATE_REVIEW' AND status IN ('OPEN','ASSIGNED','IN_PROGRESS')`,
      [
        input.principal.tenantId,
        intakeRecordId,
        JSON.stringify({
    ...
    if (!isDuplicate) {
      await client.query(
        `INSERT INTO safety_review_tasks (
         ...
         DO UPDATE SET status='OPEN', completed_at=NULL, outcome='{}'::jsonb, updated_at=now()`,
        [input.principal.tenantId, `triage:${intakeRecordId}`, intakeRecordId, input.principal.userId],
      );
    }
    ...
    await client.query(
      `INSERT INTO audit_events (
         tenant_id,actor_id,event_type,event_category,outcome,details
       ) VALUES ($1,$2,'DUPLICATE_GATE_FINALIZED','NEXUS_DUPLICATE_REVIEW','success',$3::jsonb)`,
      [
        input.principal.tenantId,
        input.principal.userId,
        JSON.stringify({
          ...
          humanDecision: input.humanDecision,
          selectedCandidateId: selectedId,
          selectedCaseId: selected?.candidate_case_id ?? null,
          routeTo: isDuplicate ? "DISPOSITION_DUPLICATE" : "ICSR_TRIAGE",
          rationale,
        }),
      ],
    );
