...
CREATE TABLE IF NOT EXISTS safety_sources (
  ...
  source_sha256 text NOT NULL CHECK (source_sha256 ~ '^[a-f0-9]{64}  status text NOT NULL DEFAULT 'RECEIVED'
...
CREATE TABLE IF NOT EXISTS safety_intake_records (
  ...
  source_lineage_sha256 text NOT NULL CHECK (source_lineage_sha256 ~ '^[a-f0-9]{64}  assigned_to uuid REFERENCES application_users(id) ON DELETE SET NULL,
  due_at timestamptz,
  created_by uuid REFERENCES application_users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS safety_case_versions (
  ...
  case_sha256 text NOT NULL CHECK (case_sha256 ~ '^[a-f0-9]{64}  change_reason text NOT NULL CHECK (length(trim(change_reason)) >= 10),
  created_by uuid REFERENCES application_users(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
