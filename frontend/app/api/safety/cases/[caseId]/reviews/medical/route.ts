...
import { recordMedicalReviewAction } from "@/lib/safety/case-review/case-review-service";
...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      throw new Error("A valid Medical Review action is required.");
    }
    if (typeof body.comments !== "string") {
      throw new Error("comments are required.");
    }

    const result = await recordMedicalReviewAction({
      principal,
      caseId,
